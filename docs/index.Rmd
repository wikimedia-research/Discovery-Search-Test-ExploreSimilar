...
# Add Dan David, Trey
abstract: >
  Wikimedia Engineering’s Discovery’s Search team ran an A/B test from 30 Jun 2017 to 24 Jul 2017 to assess the effectiveness of adding 'explore similar' widget to provide users with related pages, categories and suggested languages in each search result. We found that about 10% of users hover over the widget but almost no one clicked on the similar results. Users hover over the 'Related' section most often, followed by 'categories' and 'languages' sections. Since the time users spent on reading the similar information is too short, we doubt that 80% of the hover-overs happened by accident. Our analysis also showed that providing information similar to search results did not improve user engagement, but it did not hurt user experience neither. 
  Given the fact that very few users see the simiar results and the feedbacks we got from usertesting.com told us that users thought it is userful, we suggest doing another test which always shows the similar links without hover-on.
output:
  html_document:
    includes:
      after_body: suffix.html # not showing, need to fix css???
    code_folding: hide
    css: style.css
    fig_caption: yes
    fig_width: 10
...
# Add captions:
figure_caps(name = "Example", caption = "Example of explore similar widget on English Wikipedia.")
...
figure_caps(name = "Daily Hover Rate", caption = "Daily search-wise hover-over rate of experimental groups. Dashed lines mark the overall hover-over rate.")
...
To do ([Phab ticket](https://phabricator.wikimedia.org/T149809))
...
```{r "test group description", results='asis', echo=FALSE, eval=FALSE}
descriptions <- list(
...
There are around 185 searches everyday in each group, around 170 of them return at least one same-wiki result, and about 65 searches have clickthroughs. In the test group, about 20 searches have hover-over actions.
...
More than 80% of hover-over last less than 2 seconds, which means they are either not interested in the information or just hover-over by accident. 
```{r hover_time}
hover_over %>%
  group_by(hoverId) %>%
...
From `r figure_caps("Return Rate", display = "cite")`, we can see the return rates are not significantly different between groups. 
...
We can see that the distributions of search result pages' load time of the two groups almost overlap.
...
events %>%
  ...
  labs(x = "Load Time", y = "Density", title = "Distribution of search result page load time by test group") +
  theme_min()
```
